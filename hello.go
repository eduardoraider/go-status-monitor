...
func startMonitoring() {
	...
	for i := 0; i < MONITORING; i++ {
		for index, url := range urls {
			fmt.Println("Checking Webite:", index, ":", url)
			urlStatus(url)
		}
...
func urlStatus(url string) {
	...
	if response.StatusCode == 200 {
		...
		fmt.Println("Webite:", url, "was loaded successfully! Status Code", response.StatusCode)
	} else {
...
func logRegister(url string, status bool, code int) {
	...
	file.WriteString(time.Now().Format("02-01-2006 15:04:05 - ") + url + " - online: " + strconv.FormatBool(status) + " - code: " + strconv.Itoa(code) + "\n")
